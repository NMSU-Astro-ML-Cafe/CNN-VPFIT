...
model.compile(loss=['mse','mse','mse'],optimizer=keras.optimizers.RMSprop(lr=1e-7,rho=0.9))
#keras.optimizers.Adam(lr=1e-3))
#optimizer=tf.keras.optimizers.Adam(lr=1e-5))#, decay=1e-4))#,metrics=['mae'])

model.fit(x_train, [y_train[:,:3],y_train[:,3:6],y_train[:,6:9]], batch_size=batch_size,epochs=epochs,verbose=2,validation_data=(x_test, [y_test[:,:3],y_test[:,3:6],y_test[:,6:9]]))
...
for ii in range(num_classes):
    ...
    if(nn<=2):
        ...
        R2  = r2_score(y_test[:,nn],final_pred[2][:,nn_2])
        ax[ii].set_title(titles[ii])
        ax[ii].set_ylabel('the predicted values')
        ax[ii].set_xlabel('the true values')
        ax[ii].annotate('R2='+('%0.3f' % R2 )+'',xy=(0.05,0.9),xycoords=
          'axes fraction',bbox=dict(boxstyle="round",fc="w"), size=14)
        ax[ii].plot(y_test[:,nn], y_test[:,nn], 'k-') # where the trend should be
        ax[ii].plot(y_test[:,nn], final_pred[2][:,nn_2], 'bo', mfc='white', alpha=0.5)
        nn_2+=1
        nn+=1
fig.tight_layout()
fig.savefig('3cloud_cnn_epoch_40_lr_e-7_RMSprop_rho_0point9.png')
